...
void randomized_partition(int *arr, int p, int q, int *pos0, int *pos1);
...
void modified_quick_sort(int *arr ,int p, int r){
	if(p < r){
        int pos0, pos1;
		randomized_partition(arr, p, r, &pos0, &pos1);
...
void randomized_partition(int *arr, int p, int q, int *pos0, int *pos1){
    ...
    int random_1 = p + rand() % (q-p);
    int radnom_2 = p + rand() % (q-p);
    swap(arr[random_1], arr[p]);
    swap(arr[radnom_2], arr[p+1]);
    partition_first(arr, p, q, pos0, pos1);
}
